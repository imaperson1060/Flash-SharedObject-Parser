/* SharedObject parser. Only primitive types are implemented (bool, int, string).
 * Code by Arvydas B, edited for AMF0 by Ari Meisels.
 * 
...
public class SharedObjectParser
{
    public static SharedObject Parse(string filename, SharedObject so = null)
    {
        ...
        while (file.pos < file.file_size)
        {
            ...
            else if (so_value.type == SOTypes.TYPE_UNDEFINED)
            {
            ...
            {
                UInt16 val_length = file.Read16();
                so_value.string_val = file.ReadString((int)val_length);
                Console.WriteLine("\t" + so_value.string_val);
            }
            else if (so_value.type == SOTypes.TYPE_XML)
            {
                UInt32 val_length = file.Read32();
                so_value.string_val = file.ReadString((int)val_length);
                Console.WriteLine("\t" + so_value.string_val);
